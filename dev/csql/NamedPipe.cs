...
namespace csql
{
	...
	public class NamedPipe
	{
		...
		public void Create( string name )
		{
			...
			uint openMode = (uint)(OpenMode.PipeAccessDuplex | OpenMode.FileFlagOverlapped);
			uint pipeMode = (uint)(PipeMode.PipeWait | PipeMode.PipeTypeByte );
			if ( UseVistaFlags ) {
				// These flags are supported for Windows Vista or above only:
				// Make sure that the pipe does not exist.
				openMode|= (uint)OpenMode.FileFlagFirstPipeInstance;
				// No need for remote access.
				pipeMode|= (uint)PipeMode.PipeRejectRemoteClients;
			}
